...
int main(void) {
    ...
    expect_memeq(array2.data, test_eq, array2.len * array2.__da_item_size);

    // Delete/move test
    int32_t moved[3];
    mp_da_move(&array2, 0, 1, moved);
    expect_eq(array2.len, (size_t) 7, "%zu");
    memcpy(test_eq, (int32_t[]) { 69, 1, 2, 3, 4, 5, 0 }, array2.len * array2.__da_item_size);
    expect_memeq(array2.data, test_eq, array2.len * array2.__da_item_size);
    expect_eq(*moved, 67, "%d");

    mp_da_move(&array2, 1, 3, moved);
    expect_eq(array2.len, (size_t) 4, "%zu");
    memcpy(test_eq, (int32_t[]) { 69, 4, 5, 0 }, array2.len * array2.__da_item_size);
    expect_memeq(array2.data, test_eq, array2.len * array2.__da_item_size);
    ...
    memcpy(test_eq, (int32_t[]) { 0, 4, 5 }, array2.len * array2.__da_item_size);
    expect_memeq(array2.data, test_eq, array2.len * array2.__da_item_size);
    expect_eq(*moved, 69, "%d");

    mp_da_quick_move(&array2, array2.len - 1, moved);
    expect_eq(array2.len, (size_t) 2, "%zu");
    memcpy(test_eq, (int32_t[]) { 0, 4 }, array2.len * array2.__da_item_size);
    expect_memeq(array2.data, test_eq, array2.len * array2.__da_item_size);
    expect_eq(*moved, 5, "%d");
